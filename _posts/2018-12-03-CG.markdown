...
Our last stop before we begin our breakdown of the light culling compute shader is a quick overview of the data structures we'll be using. They're implemented solely on the GPU using shader storage buffer objects, so keep in mind that reads and writes have incoherent memory access and **will** require the appropriate barriers to avoid any disasters. As we describe them, try to keep in mind the following diagram. It will come in handy while you work towards building your own mental picture of how all the pieces fit in together:
...
However, unlike what's shown in the diagram, the global light index list **does *not* store the indices of the lights intersecting each cluster sequentially.** In fact, it'll store the indices in a somewhat random order. This was a huge source of confusion for me that took me way longer than it should have to realize, so I thought it would be worth mentioning. If you're wondering why we need such a convoluted data structure, the quick answer is that it plays nicely with the GPU and works well in parallel. Also, it allows both compute shaders and pixel shaders to read the same data structure and execute the same code. Lastly, it is very memory efficient since clusters can easily share the same lights since we are storing indices to the global light list instead of the lights themselves.


We now have all the pieces on the board and can take a look into what it takes to cull lights in clustered shading. Because my compute shader would not fit properly in this post I shortened it a bit in the section below, if you want to check out the real deal, as always, [here's the link.](https://github.com/Angelo1211/HybridRenderingEngine/blob/master/assets/shaders/ComputeShaders/clusterCullLightShader.comp) 
