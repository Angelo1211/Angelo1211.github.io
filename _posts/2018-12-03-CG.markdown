...
*...the **view frustum** into a 3D grid ...** 
 ...
 The field of Real-Time rendering has two major, yet opposed, driving forces. On the one hand, there's the strive for realism and a wish for ever improving image quality while, on the other hand, there is a hard constraint to deliver those images quickly, usually in less than a 1/30th of a second. The only way to bridge this gap is through the use of software optimizations that allow you to squeeze as much performance as possible from the underlying hardware.
 
Without these optimizations, it would be impossible to achieve the levels of visual fidelity we are accustomed to in modern high-profile games. So, in a sense, **efficient rendering algorithms** are at the core of all modern Real-Time rendering processes and are needed to display nearly anything interesting these days. However, of the countless possible optimizations that one could implement, there are a few notable techniques that can have a dramatic impact on performance and have been fully embraced by the graphics community.

We'll be focusing on two kinds of techniques: ones that reduce the amount of unnecessary pixel shader calls and others that focus on lowering the number of lighting computations during shading. You should assume that for most of the algorithms we'll be discussing we've already taken care of all other bottlenecks and have performed proper culling and mesh management somewhere further upstream. Other important details that come into play when rendering a mesh will also be stripped away in favor of presenting a clear picture of how and why these efficient algorithm outperform their peers.

However, there ain't no such thing as a free lunch. None of these algorithms have zero overhead or are guaranteed to improve the performance of your rendering pipeline. In fact, all of them carry some implicit overhead that might not be repaid for until you are working with some very complex scenes. So make sure to consult your ~~[primary care phycisian](https://renderdoc.org/)~~ profiler tools to assess the location of your current bottleneck. 
...
for mesh in scene
    for light in scene
        display += shade(mesh, light)
{% endhighlight %} 

In it's vanilla form, **forward shading** is a method of rendering scenes by linearly marching *forward* along the GPU pipeline. A single shader program will be completely in charge of drawing a mesh, starting from the vertex shader, passing through to the rasterizer and finally getting to the fragment shader. The animation and pseudocode above illustrate this behaviour at a glance. Per each mesh and light combination we issue one draw call; We keep making passes, repeating this process light by light and mesh by mesh, and additively blending the results until the image has been fully assembled. This process is more accurately known as **multi-pass** forward renderering.
...
For example, a scene with many dynamic lights of different kinds will overwhelm any multi-pass forward renderer due to the combinatorial growth in the number of shader programs. Even though each individual shader is simple, the overhead from shader program switching and bandwidth costs of repeatedly loading meshes quickly becomes a bottleneck. In addition, as you increase the number of meshes you will run into an issue called **overdraw**.
...
If we're only concerned with overdraw, the buffer simply has to keep track of the last visible mesh per pixel, normally by recording either its depth or position. Before shading we now can check if a mesh matches with the value recorded in the buffer, if not we discard. To summarize using pseudocode:
...
This is commonly known as a **Z Pre-pass**, and it is considered a *light* decoupling of shading and visibility for the sake of reducing the impact of overdraw. And I say *reducing* instead of *eliminating*, because overdraw still occurs when you are populating the depth buffer. However, now at a point where it does not impact performance as much. This will only hold true if generating geometry and meshes is somewhat straightforward in your engine. If geometry comes from procedural generation, heavy tessellation or some other source, incurring a second geometry pass might negate any possible benefits.

Usually, mentions of **Deferred shading** refer to a specific system that uses a larger buffer; In it, you store into a series of textures not only depth, but also all shading attributes of a scene from the camera's point of view. This varies significantly from implementation to implementation but it usually includes material data such as: surface normal, albedo/color or a specular/shininess component. The collection of these textures is commonly referred to as the **G-Buffer**. If you want to see a specific implementation [this Graphics Study](http://www.adriancourreges.com/blog/2017/12/15/mgs-v-graphics-study/) by  Adrian Courreges goes into detail about how a frame is rendered in Metal Gear Solid V, a fantastic game with a a fully deferred rendering engine.
...
//Shading & lighting
for light in scene
    display += shade(GBuffer, light)
{% endhighlight %}

Deferred shading can also be understood as a tool that allows us to trade off memory bandwidth in exchange for a reduction in computation. In the case of multi-pass setups, the G-Buffer reduces the costs of shading because it can cache vertex transformation results and interpolated vertex attributes, which we later reuse each lighting pass. 

Yet, reading and writing memory is not problem free either. Due to latency, all memory requests take time to be serviced.

in your tightest and most inner loop is always bad news, and even though GPU's have ways to hide the costs of memory reads, given enough lights, those latency hiding techniques will quickly become useless.

Yet in the process of dealing with overdraw we have incurred a new cost that we pay for in the shading & light accumulation step: reading the G-Buffer.


and something to keep in mind is that memory access is not cheap. 

In fact, if you were to go ahead and implement Deferred rendering as I've explained it so far, the performance gains would be pretty unremarkable, or in the case of large G-Buffers and many lights, performance would probably be worse. 
...
To alleviate this, we could switch to a **single-pass** forward renderer.
{% highlight c# %}
for mesh in scene
    mesh.shade(scene.lights)
{% endhighlight %} 
