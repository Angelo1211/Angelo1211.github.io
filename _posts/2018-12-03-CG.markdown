...
To be even more specific, we'll be focusing on two kinds of techniques: ones that reduce the amount of unnecessary pixel shader calls and others that focus on lowering the number of lighting computations during shading. You should assume that for most of the algorithms we'll be discussing we've already taken care of all other bottlenecks and have performed proper culling and mesh management somewhere further upstream. Other unimportant details that come into play when rendering a mesh will also be stripped away in favor of presenting a clear picture of how and why these efficient algorithm outperform their peers.
...
Let's begin by examining the most basic rendering algorithm: classic forward shading.
