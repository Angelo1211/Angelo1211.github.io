...
Efficient real-time rendering is essentially an exercise in compromise. It could take you only an evening&mdash; [ or a weekend ](http://www.realtimerendering.com/raytracing/Ray%20Tracing%20in%20a%20Weekend.pdf) &mdash; to write a simple renderer, but it can take years to make it fast and efficient in the use of it's hardware. As long as there has been hardware, we have been trying to mold our algorithms and to the hardware we run them on and efficient rendering is essentially that process but for the rendering algorithm. So in a sense, all algorithms that attempt to improve upon the simplest possible rendering algorithms could be qualified as **efficient rendering algorithms**, however as time has done its thing there have been a set of major "modes" of real-time rendering that have been accepted by the community as the major branches of efficient rendering. 

So what portion of the rendering pipeline are we trying to improve upon here? Well, mostly our goal will be to reduce the amount of work our GPU does overall, that is, improving efficiency can be done in many different ways, you could have mesh LOD systems that reduce the amount of work the GPU does, or we can have more optimized instruction that make pixel shader evaluations cheaper. Or you can even gain loads of performance by improving culling methodologies, that is through backface culling that makes you don't draw things that are not facing you. Or by performing view frustum culling that ensures you only render things taht are in your immediate field of view. 

None of what we're going to do, focuses on these strategies, so you can simply assume that for the algorithms we discuss below, most of that has already been taken care of or isn't the major bottleneck of our renderer. Our goal is to simply reduce the amount of pixel shader calls we make by various means. Mostly by tackling areas where we are producing unnecessary pixel shader calls, as we'll see. So, to repeat myself, we're trying to improve our efficiency by reducing the amount of times we evaluate our pixel shaders at a given pixel. In the limit our optimizations would give us a system where each pixel evaluates a fragment shader only once and generates the correct image without any wasted calculations. If you think about it, that essentially becomes a texture read in the limit, which also has its own limitations in terms of memory bandwidth, so even then it's not quite that simple. 

I'll begin by examining forward rendering in a simplified way, I'll strip away many of the important details that come into calculating pixel radiance for the sake of increasing the signal to noise ratio and making sure we focus on the parts that are relevant to us. So I won't discuss decals, textures, BRDF's or any form of baked lighting like IBL or lightmaps. I do this because the algorithms we're going to talk about are somewhat (although probably not completely ) independent of your specific implementation and therefore not too relevant to our discussion. However, I will try to mention if a specific algorithm impedes or hinder the implementation of some other feature along the way so that we can evaluate drawbacks in general terms too. 

It is worth mentioning too that there is no free lunch, none of these algorithms have zero overhead and will 100% of the time improve your rendering pipeline. In fact, most of them have a noticeable overhead if you attempt to apply them on really simple scenes of just a few objects and simple materials. That is because most algorithms attempt to perform some trade off with the bet that, for example, you're not fully saturating your bandwidth so it makes sense to offload intermediate results to textures so that they come from textures and therefore use more memory instead of compute units. It really might not be worth the engineering time you'll have to spend on them until your fragment shaders have become pretty large an unwieldy, so make sure to consult your ~~[primary care phycisian](https://renderdoc.org/)~~ profiler tools to assess the location of your current bottleneck. 

It is important to remark that the current GPU performance trends indicate that [compute capability outpaces memory bandwidth growth](https://newq.net/dl/pub/SA2014ManyLightIntro.pdf). given this knowledge we know that any algorithm that is bottleneck by memory bandwidth will scale worse than a compute bound one. this will come in handy when we finally get to tiled and clustered shading, where this principle is applied to reduce the high mememory bandwidth costs of pure deferred renderers.

Let's begin by examining an unoptimized rendering algorithm, classic forward shading:
